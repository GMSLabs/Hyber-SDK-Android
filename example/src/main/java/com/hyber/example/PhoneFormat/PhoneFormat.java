...
public class PhoneFormat {
    ...
    private static volatile PhoneFormat mInstance = null;

    public static PhoneFormat getInstance() {
        PhoneFormat localInstance = mInstance;
        if (localInstance == null) {
            synchronized (PhoneFormat.class) {
                localInstance = mInstance;
                if (localInstance == null) {
                    localInstance = new PhoneFormat();
                    ...
                    for (int r = 0; r < ruleCnt; r++) {
                        ...
                        if (openPos != -1) {
                            ...
                            rule.format = String.format("%s%s",
                                    rule.format.substring(0, openPos), rule.format.substring(closePos + 2));
                        }
